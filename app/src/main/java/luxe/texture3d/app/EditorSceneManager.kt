...
class EditorSceneManager(private val engine:Engine,private val scene:Scene,private val onChanged:()->Unit={}){
    data class Record(val uid:String,var name:String,val source:String,val asset:FilamentAsset,val loader:ResourceLoader,var visible:Boolean=true,var locked:Boolean=false,var added:Boolean=false)
    ...
    private fun begin(asset:FilamentAsset,uid:String,name:String,source:String,provided:ResourceLoader?):Record{
        require(uid !in records){"Duplicate scene instance UID"};val loader=provided?:ResourceLoader(engine,true);loader.asyncBeginLoad(asset);asset.releaseSourceData();ModelPlacement.placeOnGround(engine,asset)
        val record=Record(uid,name,source,asset,loader);records[uid]=record;onChanged();return record
    }
    ...
    fun setVisible(uid:String,visible:Boolean){val r=records[uid]?:return;if(r.visible==visible)return;r.visible=visible;if(visible){scene.addEntities(r.asset.entities);scene.addEntities(r.asset.lightEntities)}else{scene.removeEntities(r.asset.entities);scene.removeEntities(r.asset.lightEntities)};onChanged()}
    fun setLocked(uid:String,locked:Boolean){records[uid]?.let{it.locked=locked;onChanged()}}
    fun rename(uid:String,name:String){records[uid]?.let{it.name=name;onChanged()}}
    fun remove(uid:String){val r=records.remove(uid)?:return;scene.removeEntities(r.asset.entities);scene.removeEntities(r.asset.lightEntities);r.loader.asyncCancelLoad();r.loader.evictResourceData();r.loader.destroy();assetLoader.destroyAsset(r.asset);onChanged()}
