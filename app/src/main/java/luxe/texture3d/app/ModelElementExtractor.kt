...
class ModelElementExtractor(private val context:Context){
    ...
    suspend fun extract(assetDir:File,model:ElementEntity){
        ...
        val materials=gltf.optJSONArray("materials")?:JSONArray();val materialElements=mutableListOf<ElementEntity>()
        for(i in 0 until materials.length()){
            val material=materials.optJSONObject(i)?:continue;materialElements+=registerMaterial(model,modelRevision,i,material,textureIndexToElement)
        }
        updateModelRevisionManifest(model,modelRevision,materialElements,imageElements.values.toList())
        metadata.put("elementExtractionVersion",1).put("childTextureElements",JSONArray(imageElements.values.map{it.uid})).put("childMaterialElements",JSONArray(materialElements.map{it.uid}));val tmp=File(assetDir,"asset.json.extract.tmp");tmp.writeText(metadata.toString());File(assetDir,"asset.json").delete();check(tmp.renameTo(File(assetDir,"asset.json")))
    }
    ...
    private suspend fun updateModelRevisionManifest(model:ElementEntity,revisionUid:String,materials:List<ElementEntity>,textures:List<ElementEntity>){
        ...
        json.put("dependencies",dependencies).put("extractionVersion",1);val temp=File(file.parentFile,"${file.name}.tmp");temp.writeText(json.toString(2));file.delete();check(temp.renameTo(file))
    }
