...
object ModelPlacement {
    data class Result(val centerY: Float, val scale: Float, val worldCenter: FloatArray, val worldHalfExtent: FloatArray, val baseMatrix:FloatArray)
    ...
    fun placeOnGround(engine: Engine, asset: FilamentAsset, centerZ: Float = -4f): Result {
        ...
        return Result(scaledHalf[1],scale,floatArrayOf(0f,scaledHalf[1],centerZ),scaledHalf,matrix)
    }
}
