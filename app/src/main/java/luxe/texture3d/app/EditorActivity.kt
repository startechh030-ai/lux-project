...
class EditorActivity : AppCompatActivity(), Choreographer.FrameCallback {
    ...
    private fun showTransformDialog(){
        val record=sceneManager.selected()?:run{Toast.makeText(this,"Select a scene object first",Toast.LENGTH_SHORT).show();return};if(record.locked){Toast.makeText(this,"Selected object is locked",Toast.LENGTH_SHORT).show();return}
        val dialog=Dialog(this);dialog.requestWindowFeature(Window.FEATURE_NO_TITLE);val panel=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL;setPadding(dp(16),dp(12),dp(16),dp(12));setBackgroundResource(R.drawable.hub_dialog_bg)};panel.addView(TextView(this).apply{text="Transform • ${record.name}";textSize=16f;setTextColor(Color.WHITE);setTypeface(typeface,1)},LinearLayout.LayoutParams(-1,dp(36)))
        val euler=quaternionToEuler(record.rotation);val posFields=vectorFields(panel,"Position",record.position);val rotFields=vectorFields(panel,"Rotation °",euler);val scaleFields=vectorFields(panel,"Scale",record.scale)
        fun values(fields:Array<android.widget.EditText>,fallback:FloatArray)=FloatArray(3){i->fields[i].text.toString().toFloatOrNull()?:fallback[i]}
        fun apply(){val position=values(posFields,record.position);val rotation=quaternionFromEuler(values(rotFields,euler));val scale=values(scaleFields,record.scale).map{if(kotlin.math.abs(it)<.001f).001f else it}.toFloatArray();sceneManager.setTransform(record.uid,position,rotation,scale);dialog.dismiss()}
        val actions=LinearLayout(this).apply{orientation=LinearLayout.HORIZONTAL;gravity=Gravity.END};actions.addView(sceneControl("Reset",{sceneManager.resetTransform(record.uid);dialog.dismiss()}),LinearLayout.LayoutParams(dp(82),dp(36)));actions.addView(sceneControl("Ground",{val p=record.position.copyOf();p[1]=0f;sceneManager.setTransform(record.uid,p,record.rotation,record.scale);dialog.dismiss()}),LinearLayout.LayoutParams(dp(82),dp(36)).apply{leftMargin=dp(6)});actions.addView(sceneControl("Duplicate",{duplicateSelected();dialog.dismiss()}),LinearLayout.LayoutParams(dp(92),dp(36)).apply{leftMargin=dp(6)});actions.addView(sceneControl("Apply",{apply()}),LinearLayout.LayoutParams(dp(82),dp(36)).apply{leftMargin=dp(6)});panel.addView(actions,LinearLayout.LayoutParams(-1,dp(44)).apply{topMargin=dp(8)});dialog.setContentView(panel);dialog.show();dialog.window?.apply{setBackgroundDrawable(ColorDrawable(Color.TRANSPARENT));setLayout((resources.displayMetrics.widthPixels*.58f).toInt(),ViewGroup.LayoutParams.WRAP_CONTENT)}
    }
    private fun vectorFields(parent:LinearLayout,title:String,values:FloatArray):Array<android.widget.EditText>{val row=LinearLayout(this).apply{orientation=LinearLayout.HORIZONTAL;gravity=Gravity.CENTER_VERTICAL};row.addView(TextView(this).apply{text=title;textSize=12f;setTextColor(0xffb8c2d0.toInt())},LinearLayout.LayoutParams(dp(92),dp(40)));val fields=Array(3){i->android.widget.EditText(this).apply{setText("%.3f".format(values[i]));setSingleLine(true);inputType=android.text.InputType.TYPE_CLASS_NUMBER or android.text.InputType.TYPE_NUMBER_FLAG_DECIMAL or android.text.InputType.TYPE_NUMBER_FLAG_SIGNED;setTextColor(Color.WHITE);textSize=12f;background=getDrawable(R.drawable.hub_field_bg);setPadding(dp(7),0,dp(7),0);hint=listOf("X","Y","Z")[i]}};fields.forEach{row.addView(it,LinearLayout.LayoutParams(0,dp(38),1f).apply{leftMargin=dp(5)})};parent.addView(row,LinearLayout.LayoutParams(-1,dp(44)));return fields}
    ...
    private fun restoreSessionScene(manager:ProjectSessionManager,fallback:File?){
        ...
        try{for(i in 0 until instances.length()){
            ...
            val transform=item.optJSONObject("transform");sceneManager.setTransform(uid,jsonFloats(transform?.optJSONArray("position"),3,floatArrayOf(0f,0f,0f)),jsonFloats(transform?.optJSONArray("rotation"),4,floatArrayOf(0f,0f,0f,1f)),jsonFloats(transform?.optJSONArray("scale"),3,floatArrayOf(1f,1f,1f)))
    ...
    private fun jsonFloats(array:org.json.JSONArray?,size:Int,fallback:FloatArray)=FloatArray(size){i->array?.optDouble(i,fallback[i].toDouble())?.toFloat()?:fallback[i]}
